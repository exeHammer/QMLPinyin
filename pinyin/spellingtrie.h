...
class SpellingTrie {
 private:
  ...
  static const int kValidSplCharNum = 26;
  ...
  char *ym_buf_;
  int ym_size_;  // The size of longest Yunmu string, '\0'included.
  int ym_num_;
  ...
  SpellingNode* construct_spellings_subset(int item_start, int item_end,
                                           int level, SpellingNode *parent);
  bool build_f2h();
 ...
 public:
  ...
  bool construct(const char* spelling_arr, int item_size, int item_num,
  ...
  int get_spelling_str16(uint16 splid, char16 *splstr16,
                            int splstr16_len);
};
}
