...
class DictTrie : AtomDictBase {
 private:
  struct ParsingMark {
    int node_offset:24;
    int node_num:8;           // Number of nodes with this spelling id given
  ...
  int total_lma_num_;    // Total number of lemmas in this dictionary.
  ...
  inline int get_son_offset(const LmaNodeGE1 *node);

  // Get the offset of homonious ids for a node.
  inline int get_homo_idx_buf_offset(const LmaNodeGE1 *node);

  // Get the lemma id by the offset.
  inline LemmaIdType get_lemma_id(int id_offset);
  ...
  int fill_lpi_buffer(LmaPsbItem lpi_items[], int max_size,
                         LmaNodeLE0 *node);
 ...
 public:
  ...
  int get_lpis(const uint16 *splid_str, uint16 splid_str_len,
  ...
  int predict(const char16 *last_hzs, uint16 hzs_len,
  ...
  int get_total_lemma_count() {return 0;}
  void set_total_lemma_count_of_others(int count);
  ...
  int predict_top_lmas(int his_len, NPredictItem *npre_items,
