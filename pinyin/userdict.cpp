...
int UserDict::get_lpis(const uint16 *splid_str, uint16 splid_str_len,
                          LmaPsbItem *lpi_items, int lpi_max) {
  return _get_lpis(splid_str, splid_str_len, lpi_items, lpi_max, NULL);
}

int UserDict::_get_lpis(const uint16 *splid_str,
                           uint16 splid_str_len, LmaPsbItem *lpi_items,
                           int lpi_max, bool * need_extend) {
  bool tmp_extend;
  if (!need_extend)
  ...
  *need_extend = false;
  ...
  while ((int)middle < max_off && !fuzzy_break && !prefix_break) {
...
int UserDict::predict(const char16 last_hzs[], uint16 hzs_len,
...
bool UserDict::reset(const char *file) {
  ...
  int wred = fwrite(&version, 1, 4, fp);
  UserDictInfo info;
...
void UserDict::write_back_lemma(int fd) {
  ...
  int need_write = kUserDictPreAlloc *
...
void UserDict::defragment(void) {
  ...
  int real_size = total_size - lemma_size_left_;
  while (dst < real_size) {
 ...
 repeat:
    ...
    for (int j = 0; j < dict_info_.lemma_count; j++) {
...
#ifdef ___SYNC_ENABLED___
    for (int j = 0; j < dict_info_.sync_count; j++) {
...
LemmaIdType UserDict::_put_lemma(char16 lemma_str[], uint16 splids[],
  ...
  if (off != -1) {
    ...
    if (lemma_count_left_ == 0 ||
        lemma_size_left_ < (int)(2 + (lemma_len << 2))) {
...
int UserDict::get_total_lemma_count() {
  return dict_info_.total_nfreq;
}

void UserDict::set_total_lemma_count_of_others(int count) {
