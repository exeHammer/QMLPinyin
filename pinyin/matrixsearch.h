...
static const int kMaxRowNum = kMaxSearchSteps;
...
class MatrixSearch {
 private:
  ...
  static const int kMaxLmaPsbItems = 1450;

  // How many rows for each step.
  static const int kMaxNodeARow = 5;

  // The maximum length of the sentence candidates counted in chinese
  // characters
  static const int kMaxSentenceLength = 16;

  // The size of the matrix node pool.
  static const int kMtrxNdPoolSize = 200;

  // The size of the DMI node pool.
  static const int kDmiPoolSize = 800;
  ...
  // candidate.
  int spl_id_num_;                // Number of splling ids
  ...
  bool reset_search(int ch_pos, bool clear_fixed_this_step,
                    bool clear_dmi_this_step, bool clear_mtrx_this_step);

  // Delete a part of the content in pys_.
  void del_in_pys(int start, int len);
  ...
  // the fixed lemmas.
  void merge_fixed_lmas(int del_spl_pos);
  ...
  int get_lpis(const uint16* splid_str, int splid_str_len,
  ...
  int extend_dmi(DictExtPara *dep, DictMatchInfo *dmi_s);

  // Extend dmi for the composing phrase.
  int extend_dmi_c(DictExtPara *dep, DictMatchInfo *dmi_s);
  ...
  // calling this function if necessary.
  // return 0 always.
  int extend_mtrx_nd(MatrixNode *mtrx_nd, LmaPsbItem lpi_items[],
                        int lpi_num, PoolPosType dmi_fr, int res_row);


  // Try to find a dmi node at step_to position, and the found dmi node should
  // match the given spelling id strings.
  PoolPosType match_dmi(int step_to, uint16 spl_ids[], uint16 spl_id_num);
  ...
  int inner_predict(const char16 fixed_scis_ids[], uint16 scis_num,
 ...
 public:
  ...
  void set_max_lens(int max_sps_len, int max_hzs_len);
  ...
  int search(const char *py, int py_len);
  ...
  int get_spl_start(const uint16 *&spl_start);

  // Get one candiate string. If full sentence candidate is available, it will
  // be the first one.
  char16* get_candidate(int cand_id, char16 *cand_str, int max_len);
  ...
  char16* get_candidate0(char16* cand_str, int max_len,
  ...
  int cancel_last_choice();

  // Get the length of fixed Hanzis.
  int get_fixedlen();

  int get_predicts(const char16 fixed_buf[],
                      char16 predict_buf[][kMaxPredictSize + 1],
                      int buf_len);
};
}
