...
inline int DictTrie::get_son_offset(const LmaNodeGE1 *node) {
  return ((int)node->son_1st_off_l + ((int)node->son_1st_off_h << 16));
}

inline int DictTrie::get_homo_idx_buf_offset(const LmaNodeGE1 *node) {
  return ((int)node->homo_idx_buf_off_l +
          ((int)node->homo_idx_buf_off_h << 16));
}

inline LemmaIdType DictTrie::get_lemma_id(int id_offset) {
  LemmaIdType id = 0;
...
bool DictTrie::save_dict(FILE *fp) {
  ...
  if (fwrite(&lma_node_num_le0_, sizeof(int), 1, fp) != 1)
    return false;

  if (fwrite(&lma_node_num_ge1_, sizeof(int), 1, fp) != 1)
    return false;

  if (fwrite(&lma_idx_buf_len_, sizeof(int), 1, fp) != 1)
    return false;

  if (fwrite(&top_lmas_num_, sizeof(int), 1, fp) != 1)
    return false;
...
bool DictTrie::load_dict(FILE *fp) {
  ...
  if (fread(&lma_node_num_le0_, sizeof(int), 1, fp) != 1)
    return false;

  if (fread(&lma_node_num_ge1_, sizeof(int), 1, fp) != 1)
    return false;

  if (fread(&lma_idx_buf_len_, sizeof(int), 1, fp) != 1)
    return false;

  if (fread(&top_lmas_num_, sizeof(int), 1, fp) != 1 ||
  ...
  int buf_size = SpellingTrie::get_instance().get_spelling_num() + 1;
  ...
  for (uint16 splid = last_splid + 1;
    ...
    assert(static_cast<int>(splid - kFullSplIdStart) < buf_size);
...
int DictTrie::fill_lpi_buffer(LmaPsbItem lpi_items[], int lpi_max,
                                 LmaNodeLE0 *node) {
  int lpi_num = 0;
  NGram& ngram = NGram::get_instance();
  for (int homo = 0; homo < (int)node->num_of_homo; homo++) {
...
int DictTrie::fill_lpi_buffer(LmaPsbItem lpi_items[], int lpi_max,
                                 int homo_buf_off, LmaNodeGE1 *node,
                                 uint16 lma_len) {
  int lpi_num = 0;
  NGram& ngram = NGram::get_instance();
  for (int homo = 0; homo < (int)node->num_of_homo; homo++) {
...
MileStoneHandle DictTrie::extend_dict(MileStoneHandle from_handle,
                                      ...
                                      LmaPsbItem *lpi_items, int lpi_max,
                                      int *lpi_num) {
  if (NULL == dep)
    return 0;
...
MileStoneHandle DictTrie::extend_dict0(MileStoneHandle from_handle,
  ...
  *lpi_num = 0;
  ...
  int son_start = splid_le0_index_[id_start - kFullSplIdStart];
  int son_end = splid_le0_index_[id_start + id_num - kFullSplIdStart];
  for (int son_pos = son_start; son_pos < son_end; son_pos++) {
...
bool DictTrie::try_extend(const uint16 *splids, uint16 splid_num,
  ...
  if (1 == splid_num) {
    LmaNodeLE0* node_le0 = reinterpret_cast<LmaNodeLE0*>(node);
    int num_of_homo = (int)node_le0->num_of_homo;
    for (int homo_pos = 0; homo_pos < num_of_homo; homo_pos++) {
    ...
    LmaNodeGE1* node_ge1 = reinterpret_cast<LmaNodeGE1*>(node);
    int num_of_homo = (int)node_ge1->num_of_homo;
    for (int homo_pos = 0; homo_pos < num_of_homo; homo_pos++) {
      int node_homo_off = get_homo_idx_buf_offset(node_ge1);
...
int DictTrie::get_lpis(const uint16* splid_str, uint16 splid_str_len,
...
#define MAX_EXTENDBUF_LEN 200
  ...
  int* node_buf1[MAX_EXTENDBUF_LEN];  // use int for data alignment
  int* node_buf2[MAX_EXTENDBUF_LEN];
  ...
  while (spl_pos < splid_str_len) {
    ...
    if (0 == spl_pos) {  // From LmaNodeLE0 (root) to LmaNodeLE0 nodes
      for (int node_fr_pos = 0; node_fr_pos < node_fr_num; node_fr_pos++) {
      ...
      for (int node_fr_pos = 0; node_fr_pos < node_fr_num; node_fr_pos++) {
        LmaNodeLE0 *node = node_fr_le0[node_fr_pos];
        for (int son_pos = 0; son_pos < (int)node->num_of_son;
             son_pos++) {
          assert(node->son_1st_off <= lma_node_num_ge1_);
  ...
  for (int node_pos = 0; node_pos < node_to_num; node_pos++) {
    int num_of_homo = 0;
    if (spl_pos <= 1) {  // Get from LmaNodeLE0 nodes
      LmaNodeLE0* node_le0 = node_to_le0[node_pos];
      num_of_homo = (int)node_le0->num_of_homo;
      for (int homo_pos = 0; homo_pos < num_of_homo; homo_pos++) {
        int ch_pos = lma_num + homo_pos;
        lma_buf[ch_pos].id =
      ...
      num_of_homo = (int)node_ge1->num_of_homo;
      for (int homo_pos = 0; homo_pos < num_of_homo; homo_pos++) {
        int ch_pos = lma_num + homo_pos;
        int node_homo_off = get_homo_idx_buf_offset(node_ge1);
...
void DictTrie::set_total_lemma_count_of_others(int count) {
  NGram& ngram = NGram::get_instance();
...
int DictTrie::predict_top_lmas(int his_len, NPredictItem *npre_items,
                                  int npre_max, int b4_used) {
  NGram &ngram = NGram::get_instance();

  int item_num = 0;
  int top_lmas_id_offset = lma_idx_buf_len_ / kLemmaIdSize - top_lmas_num_;
  int top_lmas_pos = 0;
...
int DictTrie::predict(const char16 *last_hzs, uint16 hzs_len,
                         NPredictItem *npre_items, int npre_max,
                         int b4_used) {
