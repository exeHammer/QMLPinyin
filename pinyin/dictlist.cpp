...
bool DictList::alloc_resource(int buf_size, int scis_num) {
  // Allocate memory
...
bool DictList::init_list(const SingleCharItem *scis, int scis_num,
                         const LemmaEntry *lemma_arr, int lemma_num) {
  ...
  // calculate the size
  int buf_size = calculate_size(lemma_arr, lemma_num);
  if (0 == buf_size)
    return false;
...
int DictList::calculate_size(const LemmaEntry* lemma_arr, int lemma_num) {
  int last_hz_len = 0;
  int list_size = 0;
  int id_num = 0;
  ...
  for (int i = 0; i < lemma_num; i++) {
    if (0 == i) {
      ...
      int current_hz_len = lemma_arr[i].hz_str_len;
      ...
      if (current_hz_len == last_hz_len) {
        ...
        for (int len = last_hz_len; len < current_hz_len - 1; len++) {
  ...
  for (int i = last_hz_len; i <= kMaxLemmaSize; i++) {
    if (0 == i) {
      start_pos_[0] = 0;
      start_id_[0] = 1;
    } else {
...
void DictList::fill_scis(const SingleCharItem *scis, int scis_num) {
  assert(scis_num_ == scis_num);

  for (int pos = 0; pos < scis_num_; pos++) {
    scis_hz_[pos] = scis[pos].hz;
    scis_splid_[pos] = scis[pos].splid;
  }
}

void DictList::fill_list(const LemmaEntry* lemma_arr, int lemma_num) {
  int current_pos = 0;
...
char16* DictList::find_pos_startedbyhzs(const char16 last_hzs[],
    int word_len, int (*cmp_func)(const void *, const void *)) {
...
int DictList::predict(const char16 last_hzs[], uint16 hzs_len,
  ...
  for (uint16 pre_len = 1; pre_len <= kMaxPredictSize + 1 - hzs_len;
    ...
    while (w_buf < buf_ + start_pos_[word_len] &&
      ...
      npre_items[item_num].psb =
        ngram.get_uni_psb((int)(w_buf - buf_ - start_pos_[word_len - 1])
...
uint16 DictList::get_lemma_str(LemmaIdType id_lemma, char16 *str_buf,
  ...
  for (uint16 i = 0; i < kMaxLemmaSize; i++) {
    ...
    if (start_id_[i] <= id_lemma && start_id_[i + 1] > id_lemma) {
      int id_span = id_lemma - start_id_[i];
...
LemmaIdType DictList::get_lemma_id(const char16 *str, uint16 str_len) {
  ...
  assert(found > buf_);
  assert(static_cast<int>(found - buf_) >= start_pos_[str_len - 1]);
...
bool DictList::save_list(FILE *fp) {
  ...
  if (fwrite(&scis_num_, sizeof(int), 1, fp) != 1)
    return false;

  if (fwrite(start_pos_, sizeof(int), kMaxLemmaSize + 1, fp) !=
      kMaxLemmaSize + 1)
    return false;

  if (fwrite(start_id_, sizeof(int), kMaxLemmaSize + 1, fp) !=
      kMaxLemmaSize + 1)
    return false;
...
bool DictList::load_list(FILE *fp) {
  ...
  if (fread(&scis_num_, sizeof(int), 1, fp) != 1)
    return false;

  if (fread(start_pos_, sizeof(int), kMaxLemmaSize + 1, fp) !=
      kMaxLemmaSize + 1)
    return false;

  if (fread(start_id_, sizeof(int), kMaxLemmaSize + 1, fp) !=
      kMaxLemmaSize + 1)
    return false;
