...
void SpellingTrie::free_son_trie(SpellingNode* node) {
  ...
  for (int pos = 0; pos < node->num_of_son; pos++) {
...
bool SpellingTrie::construct(const char* spelling_arr, int item_size,
                             int item_num, float score_amplifier,
...
bool SpellingTrie::build_ym_info() {
  ...
  int ym_item_size;  // '\0' is included
  int ym_num;
  const char* ym_buf;
...
SpellingNode* SpellingTrie::construct_spellings_subset(
    int item_start, int item_end, int level, SpellingNode* parent) {
  ...
  for (int i = item_start + 1; i < item_end; i++) {
    ...
    if (char_current != char_for_node) {
      ...
      if (spelling_last_start[level + 1] != '\0' || i - item_start_next > 1) {
        int real_start = item_start_next;
        if (spelling_last_start[level + 1] == '\0')
          real_start++;
...
bool SpellingTrie::load_spl_trie(FILE *fp) {
  ...
  if (fread(&spelling_size_, sizeof(int), 1, fp) != 1)
    return false;

  if (fread(&spelling_num_, sizeof(int), 1, fp) != 1)
    return false;
...
int SpellingTrie::get_spelling_num() {
...
const char16* SpellingTrie::get_spelling_str16(uint16 splid) {
  ...
  if (splid >= kFullSplIdStart) {
    splid -= kFullSplIdStart;
    for (int pos = 0; pos < spelling_size_; pos++) {
...
int SpellingTrie::get_spelling_str16(uint16 splid, char16 *splstr16,
                                        int splstr16_len) {
  if (NULL == splstr16 || splstr16_len < kMaxPinyinSize + 1) return 0;

  if (splid >= kFullSplIdStart) {
    splid -= kFullSplIdStart;
    for (int pos = 0; pos <= kMaxPinyinSize; pos++) {
