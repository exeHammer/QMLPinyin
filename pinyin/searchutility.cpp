...
int cmp_lpi_with_unified_psb(const void *p1, const void *p2) {
  ...
  // precision.
  int up1 = item1->psb * (item2->lma_len);
  int up2 = item2->psb * (item1->lma_len);
  if (up1 < up2) {
    return -1;
  }
  if (up1 > up2) {
...
int remove_duplicate_npre(NPredictItem *npre_items, int npre_num) {
  if (NULL == npre_items || 0 == npre_num)
    return 0;

  myqsort(npre_items, npre_num, sizeof(NPredictItem), cmp_npre_by_hanzi_score);

  int remain_num = 1;  // The first one is reserved.
  for (int pos = 1; pos < npre_num; pos++) {
