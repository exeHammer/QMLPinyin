...
const int kMaxSpellingNum = 512 - kHalfSpellingIdNum - 1;
const int kMaxSearchSteps = 40;

// One character predicts its following characters.
const int kMaxPredictSize = (kMaxLemmaSize - 1);

// LemmaIdType must always be int.
typedef int LemmaIdType;
const int kLemmaIdSize = 3;  // Actually, a Id occupies 3 bytes in storage.
const int kLemmaIdComposing = 0xffffff;

typedef uint16 LmaScoreType;
typedef uint16 KeyScoreType;

// Number of items with highest score are kept for prediction purpose.
const int kTopScoreLemmaNum = 10;

const int kMaxPredictNumByGt3 = 1;
const int kMaxPredictNumBy3 = 2;
const int kMaxPredictNumBy2 = 2;
