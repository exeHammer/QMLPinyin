...
const int kMaxSpellingSize = kMaxPinyinSize;
...
class SpellingTable {
 private:
  ...
  int get_hash_pos(const char* spelling_str);
  int hash_pos_next(int hash_pos);
  void free_resource();
 public:
  SpellingTable();
  ~SpellingTable();
  ...
  bool init_table(int pure_spl_size, int spl_max_num, bool need_score);
  ...
  const char* arrange(int *item_size, int *spl_num);
