...
#define COARSE_UTC(year, month, day, hour, minute, second) \
...
#ifdef ___SYNC_ENABLED___
  uint32 * syncs_;
  int sync_count_size_;
#endif
  uint32 * offsets_by_id_;

  int lemma_count_left_;
  int lemma_size_left_;
  ...
  int _get_lpis(const uint16 *splid_str, uint16 splid_str_len,
                   LmaPsbItem *lpi_items, int lpi_max, bool * need_extend);
