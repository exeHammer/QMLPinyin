...
extern "C" {
  ...
  static const int kMaxPredictNum = 500;
  ...
  void im_set_max_lens(int max_sps_len, int max_hzs_len) {
    if (NULL != matrix_search) {
  ...
  // To be updated.
  int im_search(const char* pybuf, int pylen) {
    if (NULL == matrix_search)
      return 0;
  ...
  int im_delsearch(int pos, bool is_pos_in_splid,
  ...
  // To be removed
  int im_add_letter(char ch) {
    return 0;
  }

  const char* im_get_sps_str(int *decoded_len) {
    if (NULL == matrix_search)
      return NULL;
  ...
  char16* im_get_candidate(int cand_id, char16* cand_str,
                        int max_len) {
    if (NULL == matrix_search)
      return NULL;
  ...
  int im_get_spl_start_pos(const uint16 *&spl_start) {
    if (NULL == matrix_search)
      return 0;
  ...
  int im_get_predicts(const char16 *his_buf,
                         char16 (*&pre_buf)[kMaxPredictSize + 1]) {
    if (NULL == his_buf)
      return 0;

    int fixed_len = utf16_strlen(his_buf);
