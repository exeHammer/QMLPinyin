...
static const int kReadBufLen = 512;
static const int kSplTableHashLen = 2000;
...
int cmp_lemma_entry_hzs(const void* p1, const void* p2) {
  int size1 = utf16_strlen(((const LemmaEntry*)p1)->hanzi_str);
  int size2 = utf16_strlen(((const LemmaEntry*)p2)->hanzi_str);
  if (size1 < size2)
    return -1;
  else if (size1 > size2)
    return 1;
...
bool DictBuilder::alloc_resource(int lma_num) {
  if (0 == lma_num)
    return false;
...
char16* DictBuilder::read_valid_hanzis(const char *fn_validhzs, int *num) {
...
bool DictBuilder::hz_in_hanzis_list(const char16 *hzs, int hzs_len,
                                    char16 hz) {
  if (NULL == hzs)
    return false;
...
bool DictBuilder::str_in_hanzis_list(const char16 *hzs, int hzs_len,
                                     const char16 *str, int str_len) {
...
void DictBuilder::get_top_lemmas() {
  ...
  for (int pos = 0; pos < lemma_num_; pos++) {
    if (0 == top_lmas_num_) {
  ...
  if (kPrintDebug0) {
    printf("\n------Top Lemmas------------------\n");
    for (int pos = 0; pos < top_lmas_num_; pos++) {
...
int DictBuilder::read_raw_dict(const char* fn_raw,
  ...
  for (int i = 0; i < max_item; i++) {
    ...
    int lemma_size = utf16_strlen(token);
    ...
    for (int hz_pos = 0; hz_pos < (int)lemma_arr_[i].hz_str_len;
         hz_pos++) {
      // Get a Pinyin
      token = utf16_strtok(to_tokenize, &token_size, &to_tokenize);
      if (NULL == token) {
...
bool DictBuilder::build_dict(const char *fn_raw,
  ...
  int lma_idx_num = homo_idx_num_eq1_ + homo_idx_num_gt1_ + top_lmas_num_;
  ...
  for (int pos = 0; pos < homo_idx_num_eq1_ + homo_idx_num_gt1_; pos++) {
    id_to_charbuf(dict_trie->lma_idx_buf_ + pos * kLemmaIdSize,
                  homo_idx_buf_[pos]);
  }

  for (int pos = homo_idx_num_eq1_ + homo_idx_num_gt1_;
       pos < lma_idx_num; pos++) {
    LemmaIdType idx =
...
void DictBuilder::id_to_charbuf(unsigned char *buf, LemmaIdType id) {
  if (NULL == buf) return;
  for (int pos = 0; pos < kLemmaIdSize; pos++) {
    (buf)[pos] = (unsigned char)(id >> (pos * 8));
  }
}

void DictBuilder::set_son_offset(LmaNodeGE1 *node, int offset) {
  node->son_1st_off_l = static_cast<uint16>(offset);
  node->son_1st_off_h = static_cast<unsigned char>(offset >> 16);
}

void DictBuilder:: set_homo_id_buf_offset(LmaNodeGE1 *node, int offset) {
...
int DictBuilder::build_scis() {
  ...
  for (int pos = 0; pos < lemma_num_; pos++) {
    int hz_num = lemma_arr_[pos].hz_str_len;
    for (int hzpos = 0; hzpos < hz_num; hzpos++) {
  ...
  int unique_scis_num = 1;
  for (int pos = 1; pos < scis_num_; pos++) {
...
bool DictBuilder::construct_subset(void* parent, LemmaEntry* lemma_arr,
  ...
  for (int i = item_start + 1; i< item_end; i++) {
  ...
  int item_start_next = item_start;
  ...
  for (int i = item_start + 1; i < item_end; i++) {
    ...
    if (spl_idx_current == spl_idx_node) {
      ...
      if (homo_num > 0) {
        ...
        for (int homo_pos = 0; homo_pos < homo_num; homo_pos++) {
...
void DictBuilder::stat_init() {
  memset(max_sonbuf_len_, 0, sizeof(int) * kMaxLemmaSize);
  memset(max_homobuf_len_, 0, sizeof(int) * kMaxLemmaSize);
  memset(total_son_num_, 0, sizeof(int) * kMaxLemmaSize);
  memset(total_node_hasson_, 0, sizeof(int) * kMaxLemmaSize);
  memset(total_sonbuf_num_, 0, sizeof(int) * kMaxLemmaSize);
  memset(total_sonbuf_allnoson_, 0, sizeof(int) * kMaxLemmaSize);
  memset(total_node_in_sonbuf_allnoson_, 0, sizeof(int) * kMaxLemmaSize);
  memset(total_homo_num_, 0, sizeof(int) * kMaxLemmaSize);
