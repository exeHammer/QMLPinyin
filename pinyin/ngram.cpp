...
int update_code_idx(double freqs[], int num, double code_book[],
...
double recalculate_kernel(double freqs[], int num, double code_book[],
                          CODEBOOK_TYPE *code_idx) {
  double ret = 0;

  int *item_num =  new int[kCodeBookSize];
  assert(item_num);
  memset(item_num, 0, sizeof(int) * kCodeBookSize);
...
void iterate_codes(double freqs[], int num, double code_book[],
                   CODEBOOK_TYPE *code_idx) {
  int iter_num = 0;
  double delta_last = 0;
  do {
    int changed = update_code_idx(freqs, num, code_book, code_idx);
...
bool NGram::save_ngram(FILE *fp) {
  ...
  if (fwrite(&idx_num_, sizeof(int), 1, fp) != 1)
    return false;
...
bool NGram::load_ngram(FILE *fp) {
  ...
  if (fread(&idx_num_, sizeof(int), 1, fp) != 1 )
    return false;
...
void NGram::set_total_freq_none_sys(int freq_none_sys) {
...
#ifdef ___BUILD_MODEL___
bool NGram::build_unigram(LemmaEntry *lemma_arr, int lemma_num,
  ...
  for (int pos = 0; pos < lemma_num; pos++) {
  ...
  for (int pos = 0; pos < idx_num_; pos++) {
  ...
  int freq_pos = 0;
  for (int code_pos = 0; code_pos < kCodeBookSize; code_pos++) {
    bool found = true;
