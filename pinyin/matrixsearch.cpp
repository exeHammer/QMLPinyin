...
bool MatrixSearch::alloc_resource() {
  ...
  int mtrx_nd_size = sizeof(MatrixNode) * kMtrxNdPoolSize;
  mtrx_nd_size = align_to_size_t(mtrx_nd_size) / sizeof(int);
  int dmi_size = sizeof(DictMatchInfo) * kDmiPoolSize;
  dmi_size = align_to_size_t(dmi_size) / sizeof(int);
  int matrix_size = sizeof(MatrixRow) * kMaxRowNum;
  matrix_size = align_to_size_t(matrix_size) / sizeof(int);
  int dep_size = sizeof(DictExtPara);
  dep_size = align_to_size_t(dep_size) / sizeof(int);

  // share_buf's size is determined by the buffers for search.
  share_buf_ = new int[mtrx_nd_size + dmi_size + matrix_size + dep_size];
  ...
  npre_items_len_ = (mtrx_nd_size + dmi_size + matrix_size + dep_size) *
      sizeof(int) / sizeof(NPredictItem);
  return true;
}
...
void MatrixSearch::set_max_lens(int max_sps_len, int max_hzs_len) {
  if (0 != max_sps_len)
...
bool MatrixSearch::reset_search(int ch_pos, bool clear_fixed_this_step,
...
void MatrixSearch::del_in_pys(int start, int len) {
...
int MatrixSearch::search(const char *py, int py_len) {
  if (!inited_ || NULL == py)
    return 0;
...
int MatrixSearch::delsearch(int pos, bool is_pos_in_splid,
  ...
  int c_py_len = 0;  // The length of composing phrase's Pinyin
  int del_py_len = 1;
  if (!is_pos_in_splid) {
...
int MatrixSearch::get_candidate_num() {
...
char16* MatrixSearch::get_candidate(int cand_id, char16 *cand_str,
                                    int max_len) {
...
void MatrixSearch::update_dict_freq() {
  if (NULL != user_dict_) {
    ...
    int total_freq = user_dict_->get_total_lemma_count();
...
bool MatrixSearch::try_add_cand0_to_userdict() {
  int new_cand_num = get_candidate_num();
...
//                by 1.
int MatrixSearch::choose(int cand_id) {
  if (!inited_ || 0 == pys_decoded_len_)
    return 0;
  ...
  if (0 == cand_id) {
    ...
    matrix_[spl_start_[fixed_hzs_]].mtrx_nd_fixed = mtrx_nd_pool_ +
        matrix_[spl_start_[fixed_hzs_]].mtrx_nd_pos;
    for (int pos = fixed_lmas_; pos < lma_id_num_; pos++) {
  ...
  int cand_len = lpi_items_[cand_id].lma_len;
  ...
  int step_fr = spl_start_[fixed_hzs_];
  int step_to = spl_start_[fixed_hzs_ + cand_len];

  // 3.2 Save the length of the original string.
  int pys_decoded_len = pys_decoded_len_;
...
int MatrixSearch::cancel_last_choice() {
  if (!inited_ || 0 == pys_decoded_len_)
    return 0;

  int step_start = 0;
  if (fixed_hzs_ > 0) {
    int step_end = spl_start_[fixed_hzs_];
...
void MatrixSearch::prepare_candidates() {
  ...
  lpi_total_ = 0;
  int lpi_num_full_match = 0;  // Number of items which are fully-matched.
  while (lma_size > 0) {
    int lma_num;
    lma_num = get_lpis(spl_id_ + fixed_hzs_, lma_size,
                       lpi_items_ + lpi_total_,
                       int(kMaxLmaPsbItems - lpi_total_),
...
const char* MatrixSearch::get_pystr(int *decoded_len) {
...
void MatrixSearch::get_spl_start_id() {
  ...
  for (int pos = fixed_lmas_ + 1; pos <= lma_id_num_; pos++) {
    if (pos < lma_id_num_)
  ...
  fixed_hzs_ = 0;
  for (int pos = spl_id_num_; pos > 0; pos--) {
...
int MatrixSearch::get_spl_start(const uint16 *&spl_start) {
  get_spl_start_id();
  spl_start = spl_start_;
  return spl_id_num_;
}

int MatrixSearch::extend_dmi(DictExtPara *dep, DictMatchInfo *dmi_s) {
...
int MatrixSearch::extend_dmi_c(DictExtPara *dep, DictMatchInfo *dmi_s) {
  lpi_total_ = 0;
...
int MatrixSearch::extend_mtrx_nd(MatrixNode *mtrx_nd, LmaPsbItem lpi_items[],
                                    int lpi_num, PoolPosType dmi_fr,
                                    int res_row) {
  assert(NULL != mtrx_nd);
...
PoolPosType MatrixSearch::match_dmi(int step_to, uint16 spl_ids[],
                                    uint16 spl_id_num) {
...
char16* MatrixSearch::get_candidate0(char16 *cand_str, int max_len,
                                     uint16 *retstr_len,
                                     bool only_unfixed) {
...
int MatrixSearch::get_lpis(const uint16* splid_str, int splid_str_len,
                              LmaPsbItem* lma_buf, int max_lma_buf,
                              const char16 *pfullsent, bool sort_by_psb) {
  if (splid_str_len > kMaxLemmaSize)
    return 0;

  int num1 = dict_trie_->get_lpis(splid_str, splid_str_len,
                                     lma_buf, max_lma_buf);
  int num2 = 0;
  if (NULL != user_dict_) {
  ...
  if (splid_str_len > 1) {
    ...
    int lpsi_num = (max_lma_buf - num) * sizeof(LmaPsbItem) /
        sizeof(LmaPsbStrItem);
    //assert(lpsi_num > num);
    if (num > lpsi_num) num = lpsi_num;
    lpsi_num = num;

    for (int pos = 0; pos < lpsi_num; pos++) {
...
int MatrixSearch::inner_predict(const char16 *fixed_buf, uint16 fixed_len,
  ...
  for (int len = fixed_len; len >0; len--) {
    // How many blank items are available
    int this_max = npre_items_len_ - res_total;
    int res_this;
    ...
    if (fixed_len > 1 && 1 == len && 0 == res_total) {
      ...
      for (int nlen = 2; nlen <= fixed_len; nlen++) {
  ...
  if (kPrintDebug2) {
    ...
    for (int i = 0; i < res_total; i++) {
      printf("---");
      for (int j = 0; j < kMaxPredictSize; j++) {
...
int MatrixSearch::get_predicts(const char16 fixed_buf[],
                                  char16 predict_buf[][kMaxPredictSize + 1],
                                  int buf_len) {
  int fixed_len = utf16_strlen(fixed_buf);
