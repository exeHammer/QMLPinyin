...
int get_odd_next(int value) {
  int v_next = value;
  while (true) {
    int v_next_sqrt = (int)sqrt(v_next);

    bool is_odd = true;
    for (int v_dv = 2; v_dv < v_next_sqrt + 1; v_dv++) {
      if (v_next % v_dv == 0) {
        is_odd = false;
        break;
      }
...
int SpellingTable::get_hash_pos(const char* spelling_str) {
  int hash_pos = 0;
  for (int pos = 0; pos < spelling_size_; pos++) {
    if ('\0' == spelling_str[pos])
      break;
    hash_pos += (int)spelling_str[pos];
  }
...
int SpellingTable::hash_pos_next(int hash_pos) {
  hash_pos += 123;
  hash_pos = hash_pos % spelling_max_num_;
  return hash_pos;
}
...
bool SpellingTable::init_table(int pure_spl_size, int spl_max_num,
                               bool need_score) {
...
bool SpellingTable::put_spelling(const char* spelling_str, double freq) {
  ...
  for (int pos = 0; pos < kNotSupportNum; pos++) {
  ...
  int hash_pos = get_hash_pos(spelling_str);
...
const char* SpellingTable::arrange(int *item_size, int *spl_num) {
  if (NULL == raw_spellings_ || NULL == spelling_buf_ ||
  ...
  for (int pos = 0; pos < spelling_num_; pos++) {
