...
class DictBuilder {
 private:
  ...
  int lma_nds_used_num_le0_;
  int lma_nds_used_num_ge1_;
  ...
  int max_sonbuf_len_[kMaxLemmaSize];
  int max_homobuf_len_[kMaxLemmaSize];

  int total_son_num_[kMaxLemmaSize];
  int total_node_hasson_[kMaxLemmaSize];
  int total_sonbuf_num_[kMaxLemmaSize];
  int total_sonbuf_allnoson_[kMaxLemmaSize];
  int total_node_in_sonbuf_allnoson_[kMaxLemmaSize];
  int total_homo_num_[kMaxLemmaSize];

  int sonbufs_num1_;     // Number of son buffer with only 1 son
  int sonbufs_numgt1_;   // Number of son buffer with more 1 son;

  int total_lma_node_num_;
 ...
 private:
  ...
  void set_son_offset(LmaNodeGE1 *node, int offset);

  // Update the offset of homophonies' ids for a node.
  void set_homo_id_buf_offset(LmaNodeGE1 *node, int offset);
  ...
  bool construct_subset(void* parent, LemmaEntry* lemma_arr,
                        int item_start, int item_end, int level);
  ...
  char16* read_valid_hanzis(const char *fn_validhzs, int *num);
  ...
  int read_raw_dict(const char* fn_raw, const char *fn_validhzs,
                       int max_item);

  // Try to find if a character is in hzs buffer.
  bool hz_in_hanzis_list(const char16 *hzs, int hzs_len, char16 hz);

  // Try to find if all characters in str are in hzs buffer.
  bool str_in_hanzis_list(const char16 *hzs, int hzs_len,
  ...
  bool alloc_resource(int lma_num);
