...
class DictList {
 private:
  ...
  // char16
  int start_pos_[kMaxLemmaSize + 1];

  int start_id_[kMaxLemmaSize + 1];

  int (*cmp_func_[kMaxLemmaSize])(const void *, const void *);

  bool alloc_resource(int buf_size, int scim_num);

  void free_resource();

#ifdef ___BUILD_MODEL___
  // Calculate the requsted memory, including the start_pos[] buffer.
  int calculate_size(const LemmaEntry *lemma_arr, int lemma_num);

  void fill_scis(const SingleCharItem *scis, int scis_num);

  // Copy the related content to the inner buffer
  // It should be called after calculate_size()
  void fill_list(const LemmaEntry *lemma_arr, int lemma_num);
 ...
 public:
  ...
  // ids from 1
  bool init_list(const SingleCharItem *scis, int scis_num,
